...
## 8. Prepare and Install the Cockpit MCP Client Plugin on the Client Host
...
Download the mcp-client RPM on the new client node:

```bash
curl -L -o /tmp/ansible-playbook-extension-1.0.3-1noarch.rpm   https://github.com/alex-isv/solutions-engineering/raw/main/SUSE-PM/ansible-playbook-extension-1.0.3-1.noarch.rpm
```

Install it:

```bash
sudo zypper install --allow-unsigned-rpm -y /tmp/ansible-playbook-extension-1.0.3-1.noarch.rpm
...
```bash
sudo rpm -ivh --nosignature /tmp/ansible-playbook-extension-1.0.3-1.noarch.rpm
```
