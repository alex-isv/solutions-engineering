...
root/ – files that are copied into the image root

Everything under root/ is overlaid onto / of the image.

root/opt/ansible/ (Ansible stack baked into the image)
...
root/usr/local/bin/mariadb_tui.sh
...
Shows success/failure dialog and optionally lets you run again.

This is your post-deployment control panel for MariaDB.

root/etc/cloud/cloud.cfg.d/90_mariadb_base.cfg
